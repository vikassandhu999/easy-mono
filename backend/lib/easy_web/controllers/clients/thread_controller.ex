defmodule EasyWeb.Clients.ThreadController do
  ...
  def index(conn, _params) do
    with {:ok, %{threads: threads}} <- Threads.list_client_threads(conn.assigns.ctx) do
  ...
  def show(conn, %{"id" => id}) do
    with {:ok, thread} <- Threads.get_client_thread(conn.assigns.ctx, id) do
  ...
  def create(conn, _params) do
    with {:ok, thread} <- Threads.create_client_thread(conn.assigns.ctx, conn.body_params) do
