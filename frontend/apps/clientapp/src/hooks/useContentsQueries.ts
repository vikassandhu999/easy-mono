...
import {ContentsAPI, CreateContentProps, ListContentsProps, UpdateContentProps} from '@/Api/Contents';
...
export const useContents = (params: ContentFilters) => {
    return useInfiniteQuery({
        ...
        queryFn: async ({pageParam = 1}) => {
            const {is_published, ...rest} = params;
            const result = await ContentsAPI.listContents({
                ...(rest as any),
                page: pageParam,
                page_size: params.page_size || 20,
            });
            const value = result.getValue();
            if (is_published === undefined) return value;
            return {
                ...value,
                records: value.records.filter((c) => c.is_published === is_published),
            };
        },
        getNextPageParam: (lastPage, allPages) => {
            if (lastPage.records.length < (lastPage.page_size || 20)) return undefined;
...
export const useDeleteContent = () => {
    ...
    return useMutation({
        mutationFn: async (id: string) => {
            const result = await ContentsAPI.archiveContent(id);
            return result.getValue();
        },
        onSuccess: () => {
            queryClient.invalidateQueries({queryKey: CONTENTS_QUERY_KEYS.lists()});
...
export const useToggleContentPublish = () => {
    ...
    return useMutation({
        mutationFn: async (id: string) => {
            const result = await ContentsAPI.updateContent(id, {});
            return result.getValue();
        },
        onSuccess: (_, id) => {
            queryClient.invalidateQueries({queryKey: CONTENTS_QUERY_KEYS.lists()});
