...
export const ScheduleEntriesAPI = {
    ...
    // PATCH /v1/coach/schedules/:scheduleId/entries/bulk-move
    bulkMoveEntries: async (
        scheduleId: string,
        moves: Array<{
            entry_id: string;
            day?: number;
    ...
    ): Promise<Result<{message: string; updated_count: number}>> => {
        try {
            const response = await authedClient.patch(`/v1/coach/schedules/${scheduleId}/entries/bulk-move`, {moves});
            return Result.success(response.data);
        } catch (error: unknown) {
            return Result.failure(error);
        }
    ...
    // POST /v1/coach/schedules/:scheduleId/entries/duplicate
    duplicateEntry: async (scheduleId: string, entryId: string, targetDay?: number): Promise<Result<ScheduleEntry>> => {
        try {
            const response = await authedClient.post(`/v1/coach/schedules/${scheduleId}/entries/duplicate`, {
