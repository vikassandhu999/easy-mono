...
export const SchedulesAPI = {
  ...
  assignSchedule: async (
    scheduleId: string,
    data: AssignScheduleProps
  ): Promise<Result<Schedule>> => {
    try {
      const response = await authedClient.post(
        `/v1/coach/schedules/${scheduleId}/assign`,
        data
      );
  ...
  createProgramSchedule: async (
    programId: string,
    data: CreateScheduleProps
  ): Promise<Result<Schedule>> => {
    try {
      const response = await authedClient.post(
        `/v1/coach/programs/${programId}/schedules`,
        data
      );
      return Result.success(response.data);
    } catch (error: unknown) {
  ...
  listSchedulesByCategory: async (
    ...
    try {
      const response = await authedClient.get(
        `/v1/coach/schedules/categories/${category}`,
        { params }
      );
      return Result.success(response.data);
    } catch (error: unknown) {
