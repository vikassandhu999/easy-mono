...
function MessagesList() {
  ...
  const {data, isSuccess, fetchNextPage, isFetchingNextPage, hasNextPage} = useChatMessages(
    context?.chat?.id || '',
  ...
  const syncScrollPosition = useCallback(() => {
    ...
    try {
      ...
      const isFirstLoad = scrollHeightRef.current === 0 && messages.length > 0;
  ...
  return (
    <div
      style={{
        ...
        transition: isContentVisible ? 'opacity 0.15s ease-in-out' : 'none',
