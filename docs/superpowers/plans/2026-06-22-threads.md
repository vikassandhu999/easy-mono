...
- **Ctx-first context functions (REQUIRED):** The Authenticate plug assigns `conn.assigns.ctx` = `%Easy.Ctx{business_id, user_id}` (built by `Ctx.new(business_id, user_id)`; struct defined in `lib/easy/ctx.ex`). Per the `Easy.Exercises` convention, **every public function of `Easy.Threads` takes `%Ctx{} = ctx` as its first parameter** and reads `ctx.business_id`/`ctx.user_id` internally — it does NOT take `business_id`/`user_id` as separate arguments. Controllers obtain `ctx = conn.assigns.ctx` and pass it straight through (both coach and client controllers). Schema-module functions (changesets, query builders) and the pre-existing `Easy.Clients.get_client_for_user(business_id, user_id)` still take plain ids — Ctx is a context-layer convention only; pass `ctx.business_id`/`ctx.user_id` down to them.
...
**Interfaces:**
- Consumes Task 1 schemas + `Easy.Ctx`, `Easy.Clients.get_client_for_user/2`.
- Produces (controllers in Tasks 3–4 call these; every fn takes `%Ctx{} = ctx` first):
  - `list_threads(Ctx.t(), filters :: map) :: {:ok, [Thread.t()]}`
  - `list_client_threads(Ctx.t(), client_id) :: {:ok, [Thread.t()]} | {:error, :not_found}`
  - `list_threads_for_user(Ctx.t()) :: {:ok, [Thread.t()]} | {:error, :not_found}`
  - `get_thread(Ctx.t(), thread_id) :: {:ok, Thread.t()} | {:error, :not_found}` (messages preloaded)
  - `get_thread_for_user(Ctx.t(), thread_id) :: {:ok, Thread.t()} | {:error, :not_found}`
  - `create_thread_as_coach(Ctx.t(), attrs) :: {:ok, Thread.t()} | {:error, :not_found | Ecto.Changeset.t()}`
  - `create_thread_as_client(Ctx.t(), attrs) :: {:ok, Thread.t()} | {:error, :not_found | Ecto.Changeset.t()}`
  - `update_thread(Ctx.t(), thread_id, attrs) :: {:ok, Thread.t()} | {:error, :not_found | Ecto.Changeset.t()}`
  - `add_message_as_coach(Ctx.t(), thread_id, attrs) :: {:ok, ThreadMessage.t()} | {:error, :not_found | Ecto.Changeset.t()}`
  - `add_message_as_client(Ctx.t(), thread_id, attrs) :: {:ok, ThreadMessage.t()} | {:error, :not_found | Ecto.Changeset.t()}`
...
defmodule EasyWeb.Coaches.ThreadController do
  ...
  def create(conn, _params) do
    with {:ok, thread} <- Threads.create_thread_as_coach(conn.assigns.ctx, conn.body_params) do
...
defmodule EasyWeb.Clients.ThreadMessageController do
  ...
  def create(conn, _params) do
    with {:ok, message} <-
           Threads.add_message_as_client(conn.assigns.ctx, conn.path_params["thread_id"], conn.body_params) do
      conn
      |> put_status(:created)
...
**Deferred / dependent:** the spec's note that an attention signal can create/link a thread is the **attention** plan's concern (separate, follows this). This plan exposes `Easy.Threads.create_thread_as_coach/2` (`ctx, attrs`), which attention can call to link a signal to a thread.
