...
const IngredientsField: FC<IngredientsFieldProps> = ({ form }) => {
  ...
  const fetchedIngredients = useMemo(
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (
        ...
        focusedResultIndex < fetchedIngredients.length
      ) {
        addIngredient(
          fetchedIngredients[focusedResultIndex].id,
          fetchedIngredients[focusedResultIndex].name,
        );
      ...
      setFocusedResultIndex((prev) =>
        Math.min(prev + 1, fetchedIngredients.length - 1),
      );
      ...
      <div className={classes.sectionContent}>
        ...
        <div className={classes.searchInputWrapper}>
          <TextInput
            ...
            ref={searchInputRef}
            rightSection={
              isLoading ? <Loader color="orange" size="xs" /> : null
            }
            size="sm"
            value={searchTerm}
          />
            ...
            {currentIngredients.map((ingredient, idx) => (
              ...
              >
                ...
                <TextInput
                  ...
                  ref={(el) => {
                    if (el)
                      quantityRefs.current.set(ingredient.ingredient_id, el);
                  }}
                  size="xs"
