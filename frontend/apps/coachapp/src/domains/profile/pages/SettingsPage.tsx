...
export default function SettingsPage() {
  ...
  const displayName = coach?.name ?? `${user?.first_name ?? ''} ${user?.last_name ?? ''}`.trim();
      ...
      <PageContentWrapper>
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3">
            <Avatar size="lg">
              <Avatar.Fallback className="bg-blue-100 text-blue-700 font-semibold">{initials}</Avatar.Fallback>
            </Avatar>
            <div className="flex flex-col gap-0.5 min-w-0">
              <p className="text-lg font-semibold text-foreground truncate">{displayName}</p>
              <div className="flex items-center gap-2">
                <p className="text-sm text-default-500 truncate">{displayEmail}</p>
              </div>
            </div>
          </div>
          ...
          {showLogoutConfirm ? (
            <div className="flex flex-col gap-3 rounded-xl border border-danger-200 bg-danger-50 p-4">
              <p className="text-sm text-danger-700 font-medium">Are you sure you want to logout?</p>
...
const SettingsList: FC<{onItemClick: (id: string) => void}> = ({onItemClick}) => {
  return (
    <div className="rounded-xl border border-default-200 bg-white overflow-hidden">
      {SETTINGS_ITEMS.map((item, index) => (
        <button
          ...
          style={{
            borderBottom: index < SETTINGS_ITEMS.length - 1 ? '1px solid var(--heroui-default-200, #e4e4e7)' : 'none',
          }}
          type="button"
        >
          <div className="flex items-center gap-3">
