...
const NutritionPlanTemplateItem = ({
  plan,
  onSelect,
  isSelected,
}: NutritionPlanTemplateItemProps) => {
  ...
  return (
    ...
    >
      <div className={classes.planInfo}>
        <Text className={classes.planName} fw={500} size="sm">
          {plan.name}
        </Text>
        {plan.description && (
      ...
      <div
        className={`${classes.selectIndicator} ${isSelected ? classes.selectIndicatorActive : ""}`}
      />
    </div>
  );
};
