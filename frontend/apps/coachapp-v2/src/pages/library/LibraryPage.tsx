...
export default function LibraryPage() {
  ...
  }, [filterType, foodsData?.data, normalizedQuery, recipesData?.data, sortBy]);
  ...
  const isResourceError =
    (isFoodsError && shouldLoadFoods) || (isRecipesError && shouldLoadRecipes);
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {displayedResources.map((resource) => {
            ...
            if (resource.type === "nutrition_plan") {
              return (
                <NutritionPlanCard key={resource.id} resource={resource} />
              );
            }
            if (resource.type === "workout_plan") {
              return <WorkoutPlanCard key={resource.id} resource={resource} />;
            }
            return <ExerciseCard key={resource.id} resource={resource} />;
          })}
        </div>
      ) : null}
        ...
        <Card className="border border-separator bg-surface p-6 sm:p-8">
          <div className="flex flex-col items-center justify-center gap-4 py-8">
            ...
            <div className="text-center">
              <p className="font-semibold text-foreground">
                {hasSearchQuery ? "No results" : "No resources found"}
              </p>
              <p className="text-sm text-muted">
                {hasSearchQuery
