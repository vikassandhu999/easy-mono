...
export default function TrainingPlanBuildDrawer() {
    ...
    const handleEdit = () => {
        if (trainingPlanId) {
            openDrawer(DRAWER_KEYS.TRAINING_PLAN_EDIT, {training_plan_id: trainingPlanId});
        }
    };
    ...
    const handleDuplicate = async () => {
        ...
        try {
            ...
            openDrawer(DRAWER_KEYS.TRAINING_PLAN_BUILDER, {training_plan_id: duplicatedPlan.id});
        } catch (error) {
            const errMsg = humanizeError(error);
            notifyError(errMsg);
        }
    ...
    const handleAssign = () => {
        if (trainingPlanId) {
            openDrawer(DRAWER_KEYS.ASSIGN_TRAINING_PLAN, {training_plan_id: trainingPlanId});
        }
    };
            ...
            >
                <div>
                    ...
                    <p style={{color: 'var(--mantine-color-dimmed)', fontSize: '14px'}}>
                        This action cannot be undone. All phases, workouts, and exercises in this plan will be
                        permanently removed.
                    </p>
