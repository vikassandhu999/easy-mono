...
export function formatChat(chat: any): Chat {
  return {
    ...
    most_recent_message: chat.most_recent_message ? formatMessage(chat.most_recent_message) : undefined,
  };
}
...
export interface IChatAPI {
  ...
  getMessages(chatId: string, params: {lastMessageID?: string; limit: number}): Promise<{messages: Message[]}>;
  getMessagesKeys(chatId: string): (string | object)[];
  getMe(chat: Chat): ChatParticipant;
  getOther(chat: Chat): ChatParticipant;
  sendMessage(chatId: string, messageContent: MessageContent): Promise<Message>;
