...
export const buildPlanSessionGroups = (
  plan: Plan,
  sessions: PlanSession[],
): PlanSessionGroup[] => {
  if (!plan) return [];
  ...
  if (plan.recurrence === "daily") {
    ...
    const totalDays =
      plan.duration_days ?? (maxDayOrder >= 0 ? maxDayOrder + 1 : 7);
    ...
    return Array.from({ length: dayCount }).map((_, index) => ({
      ...
      sessions: sessions.filter(
        (session) => (session.day_order ?? 0) === index,
      ),
...
export const getScheduleWindow = (session: PlanSession): null | string => {
  if (
    session.window_start_minutes == null ||
    session.window_end_minutes == null
  ) {
    return null;
  }
...
export const formatMinutes = (value: number): string =>
  dayjs().startOf("day").add(value, "minute").format("h:mm A");
