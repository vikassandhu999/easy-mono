...
import {useGetMyCoachQuery, useUpdateMyCoachMutation} from '@/services/coach/coach';
import {CoachProfileForm_zod, type CoachProfileFormValues} from '@/services/coach/coach_definition';
...
const CoachProfileStep: React.FC<CoachProfileStepProps> = ({onComplete}) => {
  ...
  const defaultName = coach?.name || (user ? `${user.first_name} ${user.last_name}`.trim() : '');

  const {control, handleSubmit, formState, watch} = useForm<CoachProfileFormValues>({
    defaultValues: {
      name: defaultName,
      title: coach?.title ?? '',
      bio: coach?.bio ?? '',
  ...
  return (
    ...
    >
      <div className="rounded-lg border border-neutral-200 bg-white p-6">
        <h2 className="mb-1 text-lg font-semibold text-neutral-900">Complete your profile</h2>
            ...
            render={({field, fieldState}) => (
              ...
              >
                ...
                <div className="mt-1 flex justify-between">
                  {fieldState.error?.message ? <FieldError>{fieldState.error.message}</FieldError> : <span />}
                  <span className={`text-xs ${wordCount > 200 ? 'text-red-500' : 'text-neutral-400'}`}>
                    {wordCount}/200 words
                  </span>
                </div>
              </TextField>
