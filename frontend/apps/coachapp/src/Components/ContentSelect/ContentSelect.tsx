...
import {BookOpenIcon, MagnifyingGlassIcon, XIcon} from '@phosphor-icons/react';
...
const ContentCard = ({content, isSelected, onToggleSelect}: ContentCardProps) => {
    const typeConfig = CONTENT_TYPES.find((type) => type.value === content.type) || {
        value: content.type,
        label: content.type,
        icon: BookOpenIcon,
...
const SelectedItems = ({selectedItems, onRemove, onClearAll}: SelectedItemsProps) => {
    ...
    const contents = useMemo(() => {
        return (
            data?.pages
                ...
                ?.filter((content) => {
                    if (contentTypeFilter !== 'all' && content.type !== contentTypeFilter) return false;
                    return true;
                }) || []
        );
