...
export function updateScroll(
    ...
    const isLoadingOlderMessages =
        currentScrollTop <= 10 && heightIncreased && hasNewMessages;
    ...
    if (isFirstLoad && hasNewMessages) {
        ...
        isScrolledToBottom ||
        (lastMessage?.sender === selfUserKind && hasNewMessages)
    ) {
...
export const formatDate = (jsDate: Date | string): string => {
    ...
    if (dayjs().isSame(date, 'week')) return date.format('dddd'); // e.g. "Monday"
    if (dayjs().isSame(date, 'year')) return date.format('MMMM D'); // e.g. "August 22"
