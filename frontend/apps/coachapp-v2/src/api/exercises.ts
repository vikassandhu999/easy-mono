...
export const exercisesApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    listExercises: build.query<
      ApiListResponse<Exercise>,
      ListExercisesParams | void
    >({
      query: (params) => {
        if (!params) {
        ...
        return {
          ...
          params: {
            ...params,
            muscle_ids: params.muscle_ids?.length
              ? params.muscle_ids.join(",")
              : undefined,
          },
        };
      },
    ...
    updateExercise: build.mutation<
      ApiResponse<Exercise>,
      { body: ExerciseUpdateRequest; id: string }
    >({
      query: ({ body, id }) => ({
    ...
    listMuscles: build.query<ApiResponse<Muscle[]>, { search?: string } | void>(
      {
        query: (params) =>
          params
            ? {
