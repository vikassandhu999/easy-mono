...
export default function ExerciseFormPage() {
  ...
  const pageTitle = useMemo(() => {
    ...
    return exerciseData?.data?.name ? `Edit ${exerciseData.data.name}` : 'Edit Exercise';
    ...
    return (
      <Card className="border border-separator bg-surface p-6">
        <div className="flex flex-col gap-3">
          <p className="font-semibold text-foreground">Could not load exercise</p>
      ...
      >
        ...
        <div className="flex flex-col-reverse gap-3 border-t border-separator pt-4 sm:flex-row sm:justify-end">
          ...
          <Button
            ...
            variant="primary"
          >
            {isSubmitting ? (isEditing ? 'Saving...' : 'Creating...') : isEditing ? 'Save exercise' : 'Create exercise'}
          </Button>
        </div>
      </form>
