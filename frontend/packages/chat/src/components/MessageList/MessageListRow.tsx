...
export const isMessageNewDay = (current: Message, previous: Message | undefined): boolean =>
  !previous || !dayjs(current.sent_at).isSame(previous.sent_at, 'day');

const isSequential = (current: Message, previous: Message | undefined): boolean => {
  if (!previous) {
    return false;
  }
  ...
  return d1.diff(d2, 'seconds') <= MAX_SEQUENTIAL_DIFF_SECONDS && d1.minute() === d2.minute();
};
...
const MessageListRow = React.forwardRef<HTMLDivElement, {message: Message; previous?: Message}>(
          ...
          >
            ...
            <div
              style={{
                ...
                color: isMe ? 'var(--mantine-color-white)' : 'var(--mantine-color-dark-7)',
                wordWrap: 'break-word',
                maxWidth: '100%',
              }}
            >
