...
const ExerciseViewDrawer = () => {
  ...
  const [duplicateExercise, { isLoading: isDuplicating }] =
    useDuplicateExercise();
        ...
        <Stack gap="lg" p="md">
            ...
            {exercise.equipment && exercise.equipment.length > 0 ? (
              <Group gap="xs">
                {exercise.equipment.map((e) => (
                  <Badge color="gray" key={e.id} variant="outline">
                    {e.name}
                  </Badge>
                ))}
              </Group>
          ...
          {isSystem && (
            <Text c="dimmed" fs="italic" size="xs">
              Note : This is a system exercise and cannot be edited. Duplicate
              it to create your own customizable version.
            </Text>
          )}
        </Stack>
      }
