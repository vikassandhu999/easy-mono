...
export type RequestPasswordResetRequest = z.infer<
  typeof RequestPasswordResetRequest_zod
>;
...
export const AuthAPI = {
  ...
  requestPasswordReset: async (
    ...
    try {
      const response = await client.post(
        "/v1/client/password/reset/request",
        data
      );
      return Result.success(response.data);
    } catch (error: unknown) {
  ...
  resetPassword: async (
    ...
    try {
      const response = await client.post(
        "/v1/client/password/reset/confirm",
        data
      );
      return Result.success(response.data);
    } catch (error: unknown) {
  ...
  ): Promise<Result<{ message: string; client: ApiClient }>> => {
    try {
