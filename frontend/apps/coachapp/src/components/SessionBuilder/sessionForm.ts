...
import {
    ...
    workoutDefinitionToItems,
} from './utils';

const SessionItemFormSchema = z.object({
    content: z.unknown().optional(),
    content_id: z.string().uuid({message: 'Content selection is required'}),
...
export const SessionFormSchema = z.object({
    ...
    items: z.array(SessionItemFormSchema).default([]),
});

export type SessionFormValues = z.infer<typeof SessionFormSchema>;
export type SessionItemFormValues = z.infer<typeof SessionItemFormSchema>;
...
const buildDefinitionPayload = (
    ...
    switch (sessionType) {
        case 'workout': {
            ...
            const normalizedItems = workoutItems ?? values.items ?? [];

            const itemsDefinition = normalizedItems.length
                ? itemsToWorkoutDefinition('workout', normalizedItems, existingDefinition)
                : null;
...
export function createDefaultFormValues(
    ...
    if (sessionType === 'workout') {
        ...
        merged.items = overrides?.items ?? base.items;
    } else {
...
export function sessionToFormValues(session: Session): SessionFormValues {
    return createDefaultFormValues(session.session_type, {
        ...
        items: session.session_type === 'workout' ? workoutDefinitionToItems(session) : [],
    });
}
...
function shouldIncludeDefinition(values: Partial<SessionFormValues>, currentSession?: Session): boolean {
    ...
    return typeChanged || hasDefinitionSettings(values) || hasItems;
}
...
export function toCreateSessionPayload(
    ...
    const definitionPayload = buildDefinitionPayload(values, {
        ...
        workoutItems: options?.workoutItems ?? values.items,
    });
...
export function toUpdateSessionPayload(
    ...
    if (values.workout_settings) {
        const settings = sanitizeWorkoutSettings(values.workout_settings);
        if (settings) {
            payload.workout_settings = settings;
        }
    }

    if (values.meal_settings) {
        const settings = sanitizeMealSettings(values.meal_settings);
        if (settings) {
            payload.meal_settings = settings;
        }
    }

    if (values.instruction_settings) {
        const settings = sanitizeInstructionSettings(values.instruction_settings);
        if (settings) {
            payload.instruction_settings = settings;
        }
    }

    if (values.measurement_settings) {
        const settings = sanitizeMeasurementSettings(values.measurement_settings);
        if (settings) {
            payload.measurement_settings = settings;
        }
    }
