import {Chat, ChatParticipant, ChatView, formatMessage, IChatAPI, Message, MessageContent} from '@easy/chat';
...
function ChatViewPage() {
    ...
    const chatApi = useMemo<IChatAPI>(() => {
        const api: IChatAPI & {unsubs: (() => void)[]} = {
            ...
            getChatKeys: function (): (object | string)[] {
                return ['chats', chatId];
            },
            getMe: function (chat: Chat): ChatParticipant {
                return {id: '', kind: 'coach', name: '', ...(chat.coach || {})};
            },
            ...
            getMessagesKeys: function (chatId: string): (object | string)[] {
            ...
            onOtherStartedTyping: function (chatId: string, callback: () => void): () => void {
                const innerCallback = (msg: WebSocketMessage<{chat_id: string; sender: string}>) => {
