...
12. **Create forms use `{replace: true}`; edit forms use `goBack()`** — Every create form (`create-*.tsx`) navigates to the new resource's detail page with `navigate(target, {replace: true})` so the empty create route is removed from history and Back skips it. Every edit form (`edit-*.tsx`) calls `goBack()` from `useGoBack(backPath)` on both Save and Cancel, so Back pops history with `backPath` as the deep-link fallback. This is mandatory for new forms — see `AGENTS.md` "Form Navigation After Save". Exceptions: `invite-client.tsx` (in-place confirmation screen, does not navigate), `storefront-editor.tsx` (live-preview editor, does not navigate), auth wizard forward steps (intentionally push so back works between steps).
...
| **Storefront**      | Hub, Visual Editor (split-view), List/Create/Edit Offers, List/Create/Edit Testimonials  | `StorefrontEditor`, `EditorPanel`, `PreviewPanel`, `HeroEditor`, `TrustStatsEditor`, `FaqEditor`, `IntakeQuestionsEditor`, `SettingsEditor`, `OffersSummary`, `TestimonialsSummary`, `OfferForm`, `OfferCard`, `TestimonialForm`, `TestimonialCard` |
